...
await checkHttp('API health', 'http://127.0.0.1:3000/api/v1/suc-khoe', (text) => {
  ...
  if (!data || typeof data !== 'object') throw new Error('Health response khong hop le.');
});
...
await checkHttp('OpenAPI', 'http://127.0.0.1:3000/openapi-json', (text) => {
  ...
  if (recommendation?.operationId !== 'layGoiYSanPhamCuaToi') {
    throw new Error('OpenAPI chua co recommendation operationId moi nhat.');
  }
});
...
if (process.env.CHECK_MOBILE !== '0') {
  await checkHttp('Expo Metro', 'http://127.0.0.1:8081/status', (text) => {
    if (!text.toLowerCase().includes('packager-status:running')) {
      throw new Error(`Metro status khong hop le: ${text.slice(0, 120)}`);
    }
  });
}
...
if (failed.length === 0) {
  ...
  console.log('✅ API + OpenAPI + Customer + Admin + Expo Metro deu san sang.');
  process.exit(0);
}
