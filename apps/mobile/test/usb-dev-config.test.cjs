...
test('Expo Go USB development path is one-command and API-client backed', () => {
  ...
  assert.match(tool, /EXPECTED_EXPO_SDK_MAJOR/);
  assert.match(tool, /dumpsys/);
  assert.match(tool, /versionName/);
  assert.match(tool, /--remove/);
  assert.match(tool, /reverse[\s\S]*--list/);
  assert.match(tool, /force-stop/);
  assert.match(tool, /EXPO_PACKAGER_PROXY_URL/);
  assert.match(tool, /EXPO_CLEAR/);
