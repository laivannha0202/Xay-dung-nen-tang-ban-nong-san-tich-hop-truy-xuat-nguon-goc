...
// Tìm adb trên cả máy PATH thiếu (Windows vừa cài platform-tools).
// Giữ 'adb' gốc nếu đã có trong PATH để không đổi hành vi trên Linux/CI.
let adbBin = 'adb';
...
function parseAdbDevices() {
  ...
  if (unauthorized.length > 0) {
    throw new Error(
      'Điện thoại đang unauthorized. Mở khóa điện thoại và bấm "Allow USB debugging".',
    );
  }
  ...
  if (physical.length === 0) {
    throw new Error(
      'ADB chưa thấy điện thoại thật. Cắm USB, bật USB debugging rồi chạy `adb devices -l`.',
    );
  }
  if (physical.length > 1) {
    const list = physical.map((item) => item.serial).join(', ');
    throw new Error(
      `Có nhiều điện thoại ADB (${list}). Chạy lại với ANDROID_SERIAL=<serial> pnpm dev:mobile:usb`,
    );
  }
  return physical[0];
}
...
function setupUsbReverse(serial) {
  ...
  try {
    ...
    exec(adbBin, ['-s', serial, 'reverse', '--remove-all']);
    for (const port of USB_REVERSE_PORTS) addReverse(serial, port);
  }
}
...
try {
  ...
  console.log(
    '✓ Giữ nguyên ứng dụng Expo Go chính thức trên điện thoại; script chỉ nối USB/Metro/API/assets.',
  );
  ...
  } else if (await isTcpPortOpen('127.0.0.1', 8081)) {
  ...
  if (reuseMetro) {
    console.log('✓ Đang dùng Metro sẵn có; giữ process Metro cũ chạy.');
    await new Promise(() => {});
  } else {
