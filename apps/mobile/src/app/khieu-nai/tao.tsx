...
function validateAsset(asset: ImagePicker.ImagePickerAsset): string | null {
 ...
 return 'Chỉ hỗ trợ ảnh JPEG, PNG hoặc WebP theo TepTinService hệ thống.';
 }
...
export default function TrangTaoKhieuNai() {
 ...
 khieuNaiTaiKhoanDetailQueryKey(complaint.id),
 complaint,
 );

 void queryClient.invalidateQueries({
 queryKey: KHIEU_NAI_TAI_KHOAN_LIST_QUERY_KEY,
 });
 },
 ...
 description="hệ thống sẽ xác minh sản phẩm thuộc đúng tài khoản và đã được giao."
 ...
 hệ thống đã gắn {mutation.data.bangChung.length} bằng chứng.
 </Text>
 ...
 hệ thống xác minh quyền sở hữu item, trạng thái đã giao, MIME file và bằng chứng trước
 khi tạo complaint.
 </Text>
 </View>
 ...
 >
 {dieuKien.coTheKhieuNai
 ? 'hệ thống xác nhận item đủ điều kiện khiếu nại.'
 ...
 <Text className="text-xs leading-5 text-muted-foreground">
 Ảnh sẽ upload trước qua `taiTepTin`; sau đó complaint chỉ gửi các `tepTinIds` hệ thống đã
 ghi nhận.
 </Text>
