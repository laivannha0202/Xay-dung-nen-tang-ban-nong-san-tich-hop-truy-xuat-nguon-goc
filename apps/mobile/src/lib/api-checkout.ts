import {
  ...
  type LayCheckoutPreviewParams,
  type ThanhPhanCheckoutUi,
...
export type ThanhPhanCheckoutMobile = ThanhPhanCheckoutUi;
export type CheckoutPreviewMobile = CheckoutPreviewDto;
...
export async function layCheckoutPreviewMobile(
  params: LayCheckoutPreviewParams = {},
): Promise<CheckoutPreviewMobile> {
  const response = await layCheckoutPreview(params, await layTuyChonBearer());
