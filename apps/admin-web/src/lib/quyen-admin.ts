...
export const DIEU_HUONG_ADMIN: MucDieuHuongAdmin[] = [
  ...
  { path: '/danh-muc-san-pham', name: 'Danh mục sản phẩm', quyen: ['danh_muc_san_pham.xem'], nhom: 'thuong-mai' },
  ...
  { path: '/khieu-nai', name: 'Khiếu nại & hoàn tiền', quyen: ['don_hang.xu_ly'], nhom: 'thuong-mai' },
  ...
  { path: '/bao-cao-truy-xuat', name: 'Báo cáo truy xuất', quyen: ['lo_san_pham.xem'], nhom: 'kho-van' },
  { path: '/su-kien-truy-xuat', name: 'Sự kiện truy xuất', quyen: ['su_kien_truy_xuat.xem'], nhom: 'kho-van' },
...
export function coTruyCapDuongDanAdmin(pathname: string, quyenNguoiDung: string[]): boolean {
  const muc = DIEU_HUONG_ADMIN.find((item) => item.path === '/' ? pathname === '/' : pathname === item.path || pathname.startsWith(`${item.path}/`));
